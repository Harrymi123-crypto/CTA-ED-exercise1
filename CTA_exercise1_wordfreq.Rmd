...
edbf_term_counts_genre $racword <- as.integer(grepl("race|ethnicity|racism|discrimination|black|white|asian|latino|hispanic|indigenous", 
                                            x = edbf_term_counts_genre$word))
...
edbf_counts_children <- edbf_term_counts_genre %>%
  ...
  mutate(year_total = sum(n)) %>%
  filter(racword==1) %>%
  summarise(sum_rac = sum(n),
            year_total= min(year_total))

ggplot(edbf_counts_children, aes(year, sum_rac / year_total, group=1)) +
  geom_line() +
  xlab("Year") +
...
ggplot(edbf_counts_both, aes(year, sum_rac / year_total, group=genre, color=genre)) + # add the variable 'genre' as a group and color argument
  geom_line() +
  xlab("Year") +
  ylab("% race-related words") +
  scale_y_continuous(labels = scales::percent_format(),
